...
async def get_or_create_user(telegram_id: int, username: str = None, 
    ...
    users in ADMIN_IDS, and users in AUTH_USERS env var."""
    async with async_session() as db:
        result = await db.execute(select(User).where(User.telegram_id == telegram_id))
        user = result.scalar_one_or_none()

        is_admin = telegram_id in ADMIN_IDS or telegram_id in settings.auth_users
...
async def handle_callback(client, callback: CallbackQuery):
    ...
    if data == "logout_all_confirm":
        ...
        async with async_session() as db:
            result = await db.execute(select(File).where(File.id == file_id, File.user_id == callback.from_user.id))
        ...
        async with async_session() as db:
            result = await db.execute(select(Folder).where(Folder.id == folder_id, Folder.user_id == callback.from_user.id))
