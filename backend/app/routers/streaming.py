...
<script>
const C=4,S=5*1024*1024,R=3;
function init(){const e=new URLSearchParams(location.search),n=e.get('id'),t=e.get('token'),r=e.get('hash');if(!n&&!r){document.getElementById('fileInfo').textContent='Missing id or hash';return}
const i=r?'hash='+encodeURIComponent(r):'id='+n+'&token='+encodeURIComponent(t||'');document.getElementById('fileInfo').textContent=(n?'ID: '+n:'Hash: '+r);const a='/api/stream/dl?'+i;document.getElementById('dlBtn').onclick=()=>s(a);document.getElementById('directDlBtn').onclick=()=>{window.location.href=a+'&direct=1'}}
async function s(e){const t=document.getElementById('progress'),n=document.getElementById('dlBtn'),r=document.getElementById('chunkGrid');t.style.display='block';n.disabled=true;n.innerHTML='<div class="spinner"></div>';document.getElementById('error').style.display='none'
let i=[];try{const a=await fetch(e,{method:'HEAD'});if(!a.ok)throw new Error('HTTP '+a.status);const o=parseInt(a.headers.get('content-length')||'0');if(!o)throw new Error('No content-length');const l=Math.ceil(o/S);r.innerHTML='';for(let c=0;c<l;c++){const d=document.createElement('div');d.className='chunk';d.textContent=c+1;r.appendChild(d);i.push(d)}
const u=Date.now();let h=0,g=0;const m=()=>{const e=Date.now()-u;if(e>0){const t=g*1e3/e;document.getElementById('speed').textContent=(t>=1e6?(t/1e6).toFixed(1)+' MB/s':(t/1e3).toFixed(1)+' KB/s')}};setInterval(m,200);const p=async e=>{let t=0;for(let n=0;n<R&&!d;n++)try{const r=await fetch(e,{headers:{Range:'bytes='+e.start+'-'+e.end}});if(!r.ok){if(r.status===416)break;continue}const i=await r.arrayBuffer();h+=i.byteLength;g+=i.byteLength;const a=e.start/S|0;i[a].classList.add('done');document.getElementById('pct').textContent=Math.round(g/o*100)+'%';m();return}catch{}}
let d=false,v=0;const f=[];for(let t=0;t<l;t++){const n=t*S,r=Math.min(n+S-1,o-1);f.push({start:n,end:r})}let y=0;while(y<f.length){const e=[];for(let t=0;t<C&&y<f.length;t++,y++)e.push(p(f[y]));await Promise.all(e)}
const b=Date.now();if(h>=o){document.getElementById('bar').style.width='100%';document.getElementById('pct').textContent='100%';const t=((b-u)/1e3).toFixed(1);n.innerHTML='<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M22 12A10 10 0 1 1 12 2a10 10 0 0 1 10 10z"/><polyline points="16 12 12 8 8 12"/><line x1="12" y1="16" x2="12" y2="8"/></svg> Download Complete';n.onclick=()=>{window.location.href=e+'&direct=1'}}else{n.disabled=false;n.innerHTML='<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Retry';const t=document.getElementById('error');t.textContent='Incomplete: '+Math.round(g/o*100)+'% downloaded. Click retry.';t.style.display='block'}}catch(e){n.disabled=false;n.innerHTML='<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Retry';const t=document.getElementById('error');t.textContent='Error: '+e.message;t.style.display='block'}
